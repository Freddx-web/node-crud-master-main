// Express
const express = require('express');
// Enrutador
const router = express.Router();
// Exportar Modelo tabla usuario
const usuariosModel = require("../models/usuarios");


//============================================================//
//                Enrutar Pag(Ver/usuario)                   //
//============================================================//
router.get('/', function (req, res, next) {

    // Se asignar el modelo(usuariosModel)
    usuariosModel
        // Funcion para obtener los datos
        .obtener()
        .then(usuarios => {
            //Renderizar pag(usuarios/ver)
            res.status(202).render("usuarios/ver", {
                usuarios: usuarios,
            });

        })
        //En caso si falla
        .catch(err => {
            return res.status(404).send("Error obteniendo usuarios");
        });

});
//============================================================//
//                Enrutar Pag(Agregar)                        //
//============================================================//
router.get('/agregar', function (req, res, next) {
    res.status(202).render("usuarios/agregar");
});

//============================================================//
//                 Enrutar Pag(Insertar)                      //
//============================================================//
router.post('/insertar', function (req, res, next) {

    /* 
    *  Obtener el nombre y email. Es lo mismo que
    *  const nombre = req.body.nombre;(Formulario)
    *  const email = req.body.email;(Formulario)
    */

    // Capturar datos
    const { nombre, email, password } = req.body;

    // validar Campos
    if (!nombre || !email || !password) {
        return res.status(404).send("No hay nombre o email");
    }
    // Si todo va bien, seguimos


    // Se asignar el modelo(usuariosModel)
    usuariosModel
        .insertar(nombre, email, password)
        .then(idusuarioInsertado => {
            res.status(202).redirect("/usuarios");
        })
        .catch(err => {
            return res.status(500).send("Error insertando usuario");
        });
});
//============================================================//
//                Enrutar Pag(eliminar)                       //
//============================================================// 
router.get('/eliminar/:id', function (req, res, next) {

    //Se asignar el modelo(usuariosModel)
    usuariosModel
        .eliminar(req.params.id)
        .then(() => {
            res.status(202).redirect("/usuarios");
        })
        .catch(err => {
            return res.status(500).send("Error eliminando");
        });
});
//============================================================//
//                Enrutar Pag(Editar)                         //
//============================================================// 
router.get('/editar/:id', function (req, res, next) {
    usuariosModel
        .obtenerPorId(req.params.id)
        .then(usuario => {
            if (usuario) {
                res.render("usuarios/editar", {
                    usuario: usuario,
                });
            } else {
                return res.status(500).send("No existe usuario con ese id");
            }
        })
        .catch(err => {
            return res.status(500).send("Error obteniendo usuario");
        });
});
//============================================================//
//                Enrutar Pag(Acualizar)                      //
//============================================================// 
router.post('/actualizar/', function (req, res, next) {
    /* 
    *  Actualizar el nombre y email. Es lo mismo que
    *  const nombre = req.body.nombre;(Formulario)
    *  const email = req.body.email;(Formulario)
    */
    const { id, nombre, email, password } = req.body;
    if (!nombre || !email || !id || !password) {
        return res.status(500).send("No hay suficientes datos");
    }
    // Si todo va bien, seguimos

    // Se asignar el modelo(usuariosModel)
    usuariosModel
        .actualizar(id, nombre, email, password)
        .then(() => {
            res.status(202).redirect("/usuarios");
        })
        .catch(err => {
            return res.status(500).send("Error actualizando usuario");
        });
});


// Exportacion Global
module.exports = router;
